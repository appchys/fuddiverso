...
function BusinessRegisterForm() {
  ...
  const isGoogleUser = searchParams.get('google') === 'true'
  ...
  // Detectar usuario autenticado con Google y pre-llenar datos
  useEffect(() => {
    if (isGoogleUser) {
  ...
  const handleLoginExistingUser = async () => {
    try {
      ...
      setErrors({ submit: 'Error al iniciar sesión. Verifica tu contraseña.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let user = currentUser // Para usuarios de Google
      
      // Solo crear usuario con email/password si NO es usuario de Google
      if (!isGoogleUser) {
      ...
      // 4. Guardar información en localStorage para la sesión
      ...
      let errorMessage = 'Error al registrar el negocio. Intenta nuevamente.'
      
      if (error.code === 'auth/email-already-in-use') {
        setEmailExistsError(true)
        setShowLoginOption(true)
        errorMessage = 'Este email ya está registrado. ¿Es tu cuenta?'
      } else if (error.code === 'auth/weak-password') {
        errorMessage = 'La contraseña es muy débil. Usa al menos 6 caracteres.'
      } else if (error.code === 'auth/invalid-email') {
        errorMessage = 'El email no es válido.'
      } else if (error.code === 'auth/operation-not-allowed') {
        errorMessage = 'La autenticación con email no está habilitada en Firebase. Ve a Firebase Console > Authentication > Sign-in method y habilita Email/Password.'
      }
