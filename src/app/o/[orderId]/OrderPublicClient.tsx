    ...
    <div className={`${embedded ? '' : 'min-h-screen '}bg-gradient-to-b from-slate-50 to-white pb-8`}>
      ...
      <div className="max-w-md mx-auto px-4 -mt-4 space-y-4 relative z-20">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {order.items?.map((item: any, index: number) => {
              const displayName = item.variant || item.product?.name || item.name || 'Producto';

              return (
                <div key={index} className="flex justify-between items-center py-2 text-sm border-b border-gray-50 last:border-0 px-1">
                  <span className="text-gray-700">
                    <span className="font-bold">{item.quantity}x</span> {displayName}
                  </span>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">
            ...
            {order.delivery?.deliveryCost > 0 && (
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-gray-700">${order.delivery?.deliveryCost?.toFixed(2)}</span>
              </div>
            )}
            {order.creditUsed > 0 && (
              <div className="flex justify-between items-center text-sm text-green-600">
                <span className="font-bold">Créditos aplicados</span>
                <span className="font-bold">-${order.creditUsed.toFixed(2)}</span>
              </div>
            )}
